...
public:
    ...
    void gen_connected_graphs()
    {
        ...
        for (int i = 0; i < N; i++)
        {
            ...
            if (!marked[i])
            {
                ...
                subGraph->aps = aps;
                subGraph->nodes = sub_graph_nodes;
                             ...
                             vector<set<int>> aps_list)
    {
        ...
        for (auto &p : paths)
        {
            ...
            for (auto &aps : aps_list)
            {
                ...
                for (auto start = common.begin(); start != end; start++)
                    sum++;
            }
            if (sum > max)
            {
    ...
    int total_connect_cost()
    {
        ...
        for (auto subGraph : connected_graphs)
        {
    ...
    int tree_connect_cost()
    {
        ...
        return t->total_connect_cost_no_repeat();
    }
    void print()
    {
        for (int i = 0; i < N; i++)
...
int main(int argc, char const *argv[])
{
    ...
    for (auto g : graphs)
    {
        ...
        int sum = 0;
        for (auto &cg : g->connected_graphs)
            sum += cg->aps.size();
